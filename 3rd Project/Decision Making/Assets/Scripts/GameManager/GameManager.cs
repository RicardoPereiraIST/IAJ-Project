...
namespace Assets.Scripts.GameManager
{
    public class GameManager : MonoBehaviour
    {
        ...
        public List<GameObject> hPotions;
        public List<GameObject> mPotions;
        ...
        public void Start()
        {
            DataManager.Instance.SetGames(numberOfGames);
            ...
            this.hPotions = GameObject.FindGameObjectsWithTag("HealthPotion").ToList();
            this.mPotions = GameObject.FindGameObjectsWithTag("ManaPotion").ToList();
        ...
        public void Update()
        {
            ...
            if(this.characterData.HP <= 0 || this.characterData.Time >= 200)
            {
                ...
                if (DataManager.Instance.GetIterations() < DataManager.Instance.GetGames())
                {
                    DataManager.Instance.AddTime(this.characterData.Time);
                    SceneManager.LoadScene(SceneManager.GetActiveScene().name);
                }
                else if(DataManager.Instance.GetIterations() == DataManager.Instance.GetGames())
                {
                    DataManager.Instance.AddTime(this.characterData.Time);
                    this.characterData.HP = -10;
                    this.characterData.Time = 400;
                }
                else
                {
                    this.GameEnd.GetComponentInChildren<Text>().text = "DONE";
                    Debug.Log(DataManager.Instance.GetWins() + " wins: out of " + DataManager.Instance.GetGames());
                    Debug.Log("Average time was: " + DataManager.Instance.GetTime() / DataManager.Instance.GetGames());
                }
            }
