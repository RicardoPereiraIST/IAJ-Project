...
namespace Assets.Scripts
{
	public class AutonomousCharacter : MonoBehaviour
	{
        ...
        public Goal LevelUpGoal { get; private set; }
        ...
        public MCTS MCTSAlgorithm { get; set; }
		...
		void Update()
		{
			if (Time.time > this.nextUpdateTime || this.GameManager.WorldChanged)
			{
                ...
                else this.MCTSAlgorithm.InitializeMCTSearch();
			}
        ...
        {
            if(this.MCTSAlgorithm.InProgress)
            {
                var action = this.MCTSAlgorithm.Run();
                if (action != null)
                {
            ...
            this.TotalProcessingTimeText.text = "Process. Time: " + this.MCTSAlgorithm.TotalProcessingTime.ToString("F");
            this.BestDiscontentmentText.text = "Best Discontentment: " + "Not used";
            this.ProcessedActionsText.text = "Act. comb. processed: " + this.MCTSAlgorithm.MaxPlayoutDepthReached;

            if (this.MCTSAlgorithm.BestFirstChild != null)
            {
                var actionText = "";
                foreach (var action in this.MCTSAlgorithm.BestActionSequence)
                {
                    actionText += "\n" + action.Name;
                }
